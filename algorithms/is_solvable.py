...
def field_is_mined(fields, not_mines, x, y, rows, cols, mines_count):
    ...
    potential_mines = [[model.NewBoolVar(f'mine_{i}_{j}') for j in range(cols)] for i in range(rows)]
    potential_board = [[model.NewIntVar(-1, 9, f'board_{i}_{j}') for j in range(cols)] for i in range(rows)]
    ...
    model.Add(sum(potential_mines[i][j] for i in range(rows) for j in range(cols)) == mines_count)
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            model.Add(potential_board[i][j] == sum(neighbors)).OnlyEnforceIf(potential_mines[i][j].Not())

    solver = cp_model.CpSolver()
    status = solver.Solve(model)

    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        return [[solver.BooleanValue(potential_mines[i][j]) for j in range(cols)] for i in range(rows)]
    else:
        return []
...
def hint_mined_fields(hint_cache_board: list[list[int]], grid: Grid, not_mines: list[list[int]]) -> list[list[int]]:
    fields, rows, cols, mines_count = grid.convert_to_save(), grid.rows, grid.columns, len(grid.mined_fields)
...
def hint_safe_fields(hint_cache_board: list[list[int]], grid: Grid, not_mines : list[list[int]]) -> list[list[int]]:
    fields, rows, cols, mines_count = grid.convert_to_save(), grid.rows, grid.columns, len(grid.mined_fields)
    temp_board = np.zeros((rows,cols))
...
def correct_hinted_board(grid: Grid, hint_cache_board: list[list[int]]) -> list[list[int]]:
