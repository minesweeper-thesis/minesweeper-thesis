...
def optimize(params):
    ...
    if classifier_iterations > -1:
        ...
        filename = f"algorithms/tests/hyperparameter_optimization/hyperparameters/{rows},{columns},{mine_count}_{classifier}{classifier_iterations}_pso_{TRIES}.hyperparameters"
    else:
        classifier_model_file = (
            f"algorithms/models/{rows},{columns},{mine_count}_{classifier}.model"
        )
        filename = f"algorithms/tests/hyperparameter_optimization/hyperparameters/{rows},{columns},{mine_count}_{classifier}_pso_{TRIES}.hyperparameters"
