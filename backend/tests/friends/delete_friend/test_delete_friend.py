...
async def test_delete_friend_removes_friendship(authenticated_clients):
    ...
    await client2.http.post(f"/friend-requests/{friend_request_id}/accept")

    del_resp = await client1.http.delete(f"/friends/{user2_id}")
...
async def test_delete_non_friend_returns_400(authenticated_clients):
    ...
    resp = await client.http.delete(f"/friends/{fake_friend_id}")
