...
async def test_delete_friend_removes_friendship(authenticated_clients):
    ...
    await client2.http.post(f"/api/friend-requests/{friend_request_id}/accept")

    del_resp = await client1.http.delete(f"/api/friends/{user2_id}")
    assert del_resp.status_code in [200, 204]

    friends_resp = await client1.http.get("/api/friends")
...
async def test_delete_non_friend_returns_400(authenticated_clients):
    ...
    resp = await client.http.delete(f"/api/friends/{fake_friend_id}")
