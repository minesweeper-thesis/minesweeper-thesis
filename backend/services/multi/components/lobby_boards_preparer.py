...
class LobbyBoardsPreparer:
    ...
    async def prepare(self, lobby: Lobby, session: MultiplayerSession):
        to_generate = session.rounds_number - len(session.rounds)

        for round_index in range(to_generate):
            await self._prepare_round_board(
                lobby, session, lobby.game_config, round_index
            )
    ...
    async def _prepare_round_board(
        ...
        board = await self._get_unsolved_or_generate_board(
            lobby, session, game_config, round_index
        )
    ...
    async def _get_unsolved_or_generate_board(
        ...
        user_ids = [user.id for user in lobby.users]
        try:
            ...
            await self._generate_board(session, game_config, round_index)
            return None

    async def _generate_board(
        self, session: MultiplayerSession, game_config: GameConfig, round_index: int
    ):
