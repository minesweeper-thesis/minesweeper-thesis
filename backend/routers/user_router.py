...
async def save_gameplay(gameplay: schemas.Gameplay, user: CurrentUser):
...
async def get_gameplays(
    user: CurrentUser, pagination_params: Params = Depends()
...
async def make_friend_request(friend_id: uuid.UUID, user: CurrentUser):
...
async def accept_friend_request(friend_request_id: uuid.UUID, user: CurrentUser):
...
async def reject_friend_request(friend_request_id: uuid.UUID, user: CurrentUser):
...
async def remove_friend(friend_id: uuid.UUID, user: CurrentUser):
