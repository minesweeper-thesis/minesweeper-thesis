...
class InMemoryMultiplayerRepository(protocols.MultiplayerRepository):
    ...
    async def get_session(self, session_id: uuid.UUID) -> MultiplayerSession:
        ...
        pending = self.pending_sessions.get(session_id, None)
        multiplayer_session = self.sessions.get(session_id, pending)
    ...
    async def save_pending(self, session: MultiplayerSession):
        ...
        self.pending_sessions[session.id] = session
        logger.info(
    ...
    async def get_pending_for_lobby(
        ...
        for session in self.pending_sessions.values():
