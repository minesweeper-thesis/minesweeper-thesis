...
const Board = forwardRef(function Board(
    ...
    const handleCommand = (cmd) => {
        ...
        switch (cmd.type) {
            ...
            //     if (Array.isArray(cmd.start) && cmd.start[0] != null) {
            //         startFieldRef.current = cmd.start;
            //         setBoard(prev => {
            //             const newB = prev.map(r => [...r]);
            //             newB[cmd.start[0]][cmd.start[1]] = State.START_FIELD;
            //             return newB;
            //         });
            //     }
            //     break;
    ...
    const handleLeftClick = (x, y) => {
        ...
        if (current === State.START_FIELD || current === State.NOT_REVEALED || current === State.HINT) {
            sendCommand({ type: "reveal_one", cell: [x, y] });
            // if (socket && socket.readyState === WebSocket.OPEN) {
            //     socket.send(JSON.stringify({ type: "reveal_one", cell: [x, y] }));
            // }
        }
    };
    ...
    const handleRightClick = (x, y) => {
        ...
        setBoard((prev) => {
            ...
            if (current === State.NOT_REVEALED) {
                ...
                // if (socket && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: "flag", cell: [x, y] }));
            } else if (current === State.FLAG) {
                newBoard[x][y] = State.NOT_REVEALED;
                setMines(prev => prev + 1);
                sendCommand({ type: "remove_flag", cell: [x, y] });
                // if (socket && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: "remove_flag", cell: [x, y] }));
            }
    ...
    const handleMultiClick = (x, y) => {
        ...
        // if (socket && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: "reveal_many", cell: [x, y] }));
    };
